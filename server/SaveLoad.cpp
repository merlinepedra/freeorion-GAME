...
namespace {
    ...
    std::string XMLSerialize(const S& stuff) {
        ...
        try {
            auto capacity = std::min(serial_str.max_size(), Pow(2, 27) - 12);
    ...
    std::string Compress(const std::string& input_str) {
        ...
        // wrap compresed buffer string in iostream::stream to receive compressed string
    ...
    template <typename S>
    void XMLSerializeCompressThreaded(boost::asio::thread_pool& thread_pool, const S& stuff, std::string& retval)
        ...
        if (!use_binary) {
            if (use_zlib_for_zml) {
                ...
                try {
                    ...
                    XMLSerializeCompressThreaded(thread_pool, universe, universe_data);
                    std::string combat_log_data;
                    XMLSerializeCompressThreaded(thread_pool, combat_log_manager, combat_log_data);
                    std::string empire_data;
                    XMLSerializeCompressThreaded(thread_pool, empire_manager, empire_data);

                    // leaving header info uncompressed
                    ...
                    XMLSerializeCompressThreaded(thread_pool, player_save_game_data, player_data);
                    std::string species_data;
                    XMLSerializeCompressThreaded(thread_pool, species_manager, species_data);
            ...
            if (ignored_save_preview_data.save_format_marker == XML_DIRECT_MARKER) {
                ...
                try {
                    if (ignored_save_preview_data.uncompressed_text_size > 0) {
                        ...
                        const auto capacity = std::min(serial_str.max_size(), Pow(2, 26) - 12); // I read on StackOverflow that Qt grows string capacity to slightly less than powers of two due to some allocators perform worse at exact powers of 2
                        serial_str.reserve(capacity);
                    }
                    if (ignored_save_preview_data.compressed_text_size > 0) {
                        ...
                        compressed_str.reserve(Pow(2, 26));
                    }
                } catch (...) {
