...
public:
    ...
    [[nodiscard]] virtual std::set<std::string> Tags(const ScriptingContext&) const { return {}; }

    /** Returns true iff this object has the tag with the indicated \a name. */
    [[nodiscard]] virtual bool                  HasTag(const std::string& name, const ScriptingContext&) const { return false; }
